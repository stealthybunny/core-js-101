...
function findFirstSingleChar(str) {
  const count = str.split('').reduce((acc, item) => {
...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  ...
  return `${startPar}${start}, ${end}${endPar}`;
}
...
function getDigitalRoot(num) {
  ...
  const sum = (str.split('').reduce((acc, item) => parseInt(acc, 10) + parseInt(item, 10)));
...
function getCommonDirectoryPath(pathes) {
  const last = pathes[pathes.length - 1].split('/');
  const zeros = pathes.map((el) => el[0]);
  while (!pathes.every((el) => el.includes(last.join('/')))) {
    last.length -= 1;
  }
  if (zeros.every((el) => el === '/')) {
    return `${last.join('/')}/`;
  }
  return '';
}
...
function getMatrixProduct(m1, m2) {
  ...
  const N = m2[0].length;
  const K = m1[0].length;
  const product = new Array(M);
  for (let m = 0; m < M; m += 1) {
    product[m] = new Array(N);
    for (let n = 0; n < N; n += 1) {
      product[m][n] = 0;
      for (let k = 0; k < K; k += 1) {
        product[m][n] += m1[m][k] * m2[k][n];
      }
    }
  }
  return product;
